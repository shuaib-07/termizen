...
export function ContextMenuContent({
  ...
  return createPortal(
    <div
      ...
      className={cn(
        "fixed z-[100] [filter:drop-shadow(0_18px_28px_rgba(0,0,0,0.5))]",
    ...
    >
      <motion.div
        ...
        className={cn(
          "min-w-56 overflow-hidden rounded-xl border border-white/15 bg-zinc-950/98 backdrop-blur-2xl p-1.5 text-foreground outline-none shadow-2xl",
          className,
        )}
      >
        {children}
...
function ContextMenuItemBase({
  ...
  return (
    <button
      ...
      className={cn(
        "relative isolate flex w-full select-none items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-[13px] outline-none transition-colors cursor-pointer",
        "focus-visible:ring-2 focus-visible:ring-foreground/15",
        "disabled:pointer-events-none disabled:opacity-40",
        inset && "pl-8",
        tone === "destructive" ? "text-red-400 hover:text-red-300" : "text-foreground",
        className,
      )}
    >
      {active ? (
...
export function ContextMenuSeparator({
  ...
  return (
    <hr className={cn("-mx-1 my-1 h-px border-0 bg-border/50", className)} />
  );
}
